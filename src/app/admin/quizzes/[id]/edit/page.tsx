...
import { formatDate, getDifficultyColor, getCategoryColor, generateId } from "@/lib/utils";
...
export default function EditQuizPage() {
  ...
  const { data: quiz, loading, error, refetch } = useFetch<QuizData>(`/api/quizzes/${id}`);

  const [form, setForm] = useState<QuizForm>({
    title: "", description: "", category: "GENERAL", difficulty: "INTERMEDIATE",
    status: "DRAFT", passingScore: 70, timeLimitMins: 15,
  });
      ...
      setQuestions(
        quiz.questions
          ...
          .map((q) => ({
            ...
            options: q.options
              .sort((a, b) => a.order - b.order)
              .map((o) => ({ id: o.id, text: o.text, isCorrect: o.isCorrect, order: o.order })),
          }))
      );
    }
  }, [quiz]);
  ...
  const handleSave = async () => {
    ...
    try {
      await apiPut(`/api/quizzes/${id}`, {
        ...
        timeLimitMins: form.timeLimitMins,
        questions: questions.map((q, qi) => ({
          ...
          options: q.options.map((o, oi) => ({
            id: o.id.startsWith("new_") ? undefined : o.id,
            text: o.text,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setForm({ ...form, timeLimitMins: e.target.value ? parseInt(e.target.value) : null })}
              className="input-field"
              ...
              {question.isExpanded && (
                <div className="ml-14 space-y-3">
                  ...
                  <div className="space-y-2">
                    <label className="label-text text-xs">Options</label>
